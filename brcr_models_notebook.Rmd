---
title: "Mark-Recapture Models for MAPS Data"
output:
  html_document:
    df_print: paged
---
...
Mark-recapture models for analysing capture history data from the MAPS program. See this **reference** for detailed model description.
...
### 1. Density dependent model with residents

```{r, message=FALSE}

{
sink("dd_model_r.jags")
  cat("
      model {
      ...
      # Part 1: Priors
        ...
        gamma[1] ~ dnorm(0,pow(10,-2))
        gamma[2] ~ dnorm(0,pow(10,-2))
        delta ~ dnorm(0,pow(10,-2))
      ...
      for (i in 1:nind) {
        ...
        for (t in first[i]:(last_pop[pop[i]]-1)) {
          phi[i,t] <- ilogit(alpha[stage[i,t]] + beta*(density[pop[i],t]-1) + epsilon[t])
      ...
      for (l in 1:nfec) {
        ...
        Njuv2[l] <- exp(log(Nad[pop_index[l],year_index[l]]) + theta + zeta*(density_fec[l]-1) + 
                        omega[l])
        w[l] ~ dbern(psi)
        Nobs_juv2[l] ~ dpois(max(py_j[pop_index[l],year_index[l]]*Njuv2[l], 0.00000001))
        Nobs_juv_pred[l] ~ dpois(max(py_j[pop_index[l],year_index[l]]*Njuv2[l], 0.00000001))
      }#l
      ...
      # Part 5: Model Checking
        ...
        for (l in 1:nfec) {
          ft_fec_data1[l] <- ((Nobs_juv2[l]-(py_j[pop_index[l],year_index[l]]*Njuv2[l]))^2)/
                              (py_j[pop_index[l],year_index[l]]*Njuv2[l])
          ft_fec_pred1[l] <- ((Nobs_juv_pred[l]-(py_j[pop_index[l],year_index[l]]*Njuv2[l]))^2)/
                              (py_j[pop_index[l],year_index[l]]*Njuv2[l])
        }#l
...
### 2. Density Dependent Model without Residents
...
### 3. Density Independent Model with Residents

```{r, message=FALSE}

{
sink("di_model_r.jags")
  cat("
      model {
