...
saveRDS(band_data, file = "data/band_data.rds")
saveRDS(effort_data, file = "data/effort_data.rds")
saveRDS(midpoint_data, file = "data/midpoint_data.rds")
saveRDS(weather_data, file = "data/weather_data.rds")
```
...
format.fec <- function(effort, Nobs) {
  ...
  pop_index2 <- which(Nobs_ad>4, arr.ind = T)[,1]
  year_index2 <- which(Nobs_ad>4, arr.ind = T)[,2]
  
  # Fecundity data used in mark-recapture models.
  fec_data1 <- Nobs_juv[which(Nobs_ad>0 & Nobs_juv>0)]
  fec_data2 <- Nobs_juv[which(Nobs_ad>4)]
...
saveRDS(chdata, file = "data/BRCR_chdata.rds")
```
